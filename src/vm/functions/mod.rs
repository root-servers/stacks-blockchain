...
fn special_if(
    ...
    runtime_cost(ClarityCostFunction::If, env, InputSize::None)?;
...
fn special_asserts(
    ...
    runtime_cost(ClarityCostFunction::Asserts, env, InputSize::None)?;
    // handle the conditional clause.
    let conditional = eval(&args[0], env, context)?;
...
fn special_contract_of(
    ...
    runtime_cost(ClarityCostFunction::ContractOf, env, InputSize::None)?;
